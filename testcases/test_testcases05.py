...
class Test_Testcases(unittest.TestCase):
    excel=Handle_Excel(os.path.join(DATA_DIR,"cases.xlsx"),"testcases")
    ...
    def test_testcases(self,case):
        ...
        elif case["title"] == "接口创建成功":
            iid=str(jsonpath.jsonpath(res,"$..id")[0])
            setattr(EnvData,"iid",iid)
        #断言
        try:
            # 调用断言的方法进行断言
    ...
    #  随机生成一个数据库未注册的用例名称
    @classmethod
    def random_testname(cls):
        while True:
            randomnum = str(random.randint(0, 10000))
            name = "登录接口"+randomnum
            sql = 'SELECT * FROM test.tb_testcases WHERE name="{}"'.format(name,encoding='utf-8')
